...
*/
	...
	for ($i=0; $i < sizeof($json); $i++){
		$id_participants = $json[$i]['id_participants'];
		$query2 = mysqli_query($con,"SELECT value FROM judgement where id_session_assesment = '4' AND id_participants = '$id_participants'") or die(mysqli_error($con));
			
			while($row2 = mysqli_fetch_assoc($query2)){
				$json[$i]['personality'] = $row2;
		}
	...
	for ($i=0; $i < sizeof($json); $i++){
		$id_participants = $json[$i]['id_participants'];
		$query3 = mysqli_query($con,"SELECT * FROM session_note where id_session_assesment = '4' AND id_participants = '$id_participants'") or die(mysqli_error($con));
